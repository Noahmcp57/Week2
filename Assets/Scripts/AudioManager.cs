...
public class AudioManager : Singelton<AudioManager>
{
    ...
    {
        base.Awake();                // ← make sure your singleton base actually runs
    ...
    private void InitializeAudioSources()
    {
        ...
        var musicGroups = audioMixer?.FindMatchingGroups("Music") ?? new AudioMixerGroup[0];
        if (musicGroups.Length > 0)
            musicSource.outputAudioMixerGroup = musicGroups[0];
        else
            Debug.LogWarning("AudioManager → no ‘Music’ group found in AudioMixer.");

        var sfxGroups = audioMixer?.FindMatchingGroups("SFX") ?? new AudioMixerGroup[0];
        if (sfxGroups.Length > 0)
            sfxSource.outputAudioMixerGroup = sfxGroups[0];
        else
            Debug.LogWarning("AudioManager → no ‘SFX’ group found in AudioMixer.");
    }
